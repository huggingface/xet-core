...
/// Builder for constructing and starting a simulation scenario.
        ...
        scenario.add_task(move |child_token| {
            tokio::spawn(async move {
                ...
                loop {
                    tokio::select! {
                        _ = interval.tick() => {
                            let total_upload = server_stats.total_upload_bytes_possible();
                            let bandwidth = server_stats.current_bandwidth().await.unwrap_or(0);
                            let latency_ms = server_stats.current_latency_ms().await;
                            let elapsed = start_instant_stats.elapsed().as_secs_f64();
                            let sample = NetworkStats {
                                ...
                                interface: None,
                                elapsed_sec: elapsed,
                                total_upload_possible: total_upload,
...
impl SimulationScenario {
    ...
    async fn wait_until_ready(&self) -> ScenarioResult<()> {
        let http_client = build_http_client("simulation_scenario", "simulation", None)
    ...
    pub async fn finish(mut self) -> ScenarioResult<()> {
        ...
        let total_upload = self.server.total_upload_bytes_possible();
        let bandwidth = self.server.current_bandwidth().await.unwrap_or(0);
        let latency_ms = self.server.current_latency_ms().await;
        let elapsed = self.start_instant.elapsed().as_secs_f64();
        let final_sample = NetworkStats {
            timestamp: chrono::Utc::now().timestamp_millis().to_string(),
            latency_ms,
            bandwidth_bytes_per_sec: bandwidth,
            congestion_mode: None,
            ...
            elapsed_sec: elapsed,
            total_upload_possible: total_upload,
            server_latency_profile: self.server_latency_profile_name.clone(),
        };
