...
use xet_client::chunk_cache::{random_key, random_range, DiskCache, RandomEntryIterator};
...
fn benchmark_cache_get<T: ChunkCacheExt>(c: &mut Criterion) {
    let cache_root = tempfile::Builder::new().prefix(format!("benchmark_cache_get_{}", T::name()).as_str()).tempdir().unwrap();
...
fn benchmark_cache_get_mt<T: ChunkCacheExt + 'static>(c: &mut Criterion) {
    let cache_root = tempfile::Builder::new().prefix(format!("benchmark_cache_get_mt_{}", T::name()).as_str()).tempdir().unwrap();
    let cache = T::_initialize(cache_root.path().to_path_buf(), CAPACITY).unwrap();
...
fn benchmark_cache_put_mt<T: ChunkCacheExt + 'static>(c: &mut Criterion) {
    let cache_root = tempfile::Builder::new().prefix(format!("benchmark_cache_put_mt_{}", T::name()).as_str()).tempdir().unwrap();
    let cache = T::_initialize(cache_root.path().to_path_buf(), CAPACITY).unwrap();
...
fn benchmark_cache_put<T: ChunkCacheExt + 'static>(c: &mut Criterion) {
    let cache_root = tempfile::Builder::new().prefix(format!("benchmark_cache_put_{}", T::name()).as_str()).tempdir().unwrap();
    let cache = T::_initialize(cache_root.path().to_path_buf(), CAPACITY).unwrap();
...
fn benchmark_cache_get_hits<T: ChunkCacheExt + 'static>(c: &mut Criterion) {
    let cache_root = tempfile::Builder::new().prefix(format!("benchmark_cache_get_hits_{}", T::name()).as_str()).tempdir().unwrap();
    let cache = T::_initialize(cache_root.path().to_path_buf(), CAPACITY).unwrap();
