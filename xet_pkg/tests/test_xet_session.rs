...
async fn upload_bytes_async(session: &XetSession, data: &[u8], name: &str) -> XetFileInfo {
    ...
    let handle = commit
        ...
        .await
        .unwrap();
    let file_meta = handle.finish().await.unwrap();
...
fn upload_bytes_sync(session: &XetSession, data: &[u8], name: &str) -> XetFileInfo {
    ...
    let handle = commit
        ...
        .unwrap();
    let file_meta = handle.finish_blocking().unwrap();
    ...
    let metas = {
        ...
        for (name, data) in &files {
            let h = commit
                ...
                .await
                .unwrap();
            metas.push(h.finish().await.unwrap());
        }
        commit.commit().await.unwrap();
        metas
    };
...
async fn async_task_status_transitions() {
    ...
    assert!(handle.get_progress().is_some() || handle.try_finish().is_none());
