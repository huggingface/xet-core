...
impl UploadCommit {
    ...
    pub async fn upload_from_path(
        ...
        if matches!(self.session.runtime_mode, RuntimeMode::External) {
            return self.inner.start_upload_file_from_path(absolute_path, sha256).await;
        }
        let inner = self.inner.clone();
        self.session
            .runtime
            .bridge_to_owned("upload_from_path", async move {
                inner.start_upload_file_from_path(absolute_path, sha256).await
            })
            .await?
    }
    ...
    pub async fn upload_file(
        ...
        if matches!(self.session.runtime_mode, RuntimeMode::External) {
            return self.inner.start_upload_file(file_name, file_size, sha256).await;
        }
        let inner = self.inner.clone();
        self.session
            .runtime
            .bridge_to_owned("upload_file", async move { inner.start_upload_file(file_name, file_size, sha256).await })
            .await?
    }
    ...
    pub async fn upload_bytes(
        ...
        if matches!(self.session.runtime_mode, RuntimeMode::External) {
            return self.inner.start_upload_bytes(bytes, tracking_name, sha256).await;
        }
        let inner = self.inner.clone();
        self.session
            .runtime
            .bridge_to_owned(
                "upload_bytes",
                async move { inner.start_upload_bytes(bytes, tracking_name, sha256).await },
            )
            .await?
    }
