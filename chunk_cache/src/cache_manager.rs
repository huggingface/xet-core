...
use crate::{ChunkCache, DiskCache};
...
/// get_cache attempts to return a cache given the provided cache directory.
/// Cache size is read from `xet_config().chunk_cache.size_bytes`.
pub fn get_cache(cache_directory: &PathBuf) -> Result<Arc<dyn ChunkCache>, ChunkCacheError> {
    CACHE_MANAGER.get(cache_directory)
}
...
impl CacheManager {
    ...
    /// get takes a cache directory path and checks if there exists a valid `DiskCache` with a matching
    ...
    /// then it creates a new instance based on the provided directory.
    fn get(&self, cache_directory: &PathBuf) -> Result<Arc<dyn ChunkCache>, ChunkCacheError> {
        let mut vals = self.vals.lock()?;
        if let Some(v) = vals.get_mut(cache_directory) {
            ...
            let result: Arc<dyn ChunkCache> = Arc::new(DiskCache::initialize(cache_directory)?);
            v.replace(Arc::downgrade(&result));
            Ok(result)
        } else {
            // create a new Cache and insert weak pointer to managed map
            let result: Arc<dyn ChunkCache> = Arc::new(DiskCache::initialize(cache_directory)?);
            vals.insert(cache_directory.clone(), RefCell::new(Arc::downgrade(&result)));
            Ok(result)
        }
    }
}
