...
//!
//! Users construct profiles only via [`NetworkProfileBuilder`], which returns a
//! [`NetworkProfile`]. When no congestion is requested, the builder returns a [`ConsistentNetworkProfile`].
//! The proxy consumes the profile
//! from the provider every 250ms.
...
/// Single builder route for network profiles. Build a provider with:
/// `NetworkProfileBuilder::new().with_bandwidth_str(...).with_latency(...).with_congestion(...).build()`.
#[derive(Clone)]
...
/// Valid strings for [`NetworkProfileBuilder::with_congestion`].
