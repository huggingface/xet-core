...
use xet_core_structures::merklehash::{DataHashHexParseError, MerkleHash, xorb_hash};
...
impl JsChunkOut {
    fn new_with_dedup(chunk: xet_data::deduplication::Chunk) -> Self {
        let hash_eligible = xet_core_structures::metadata_shard::constants::hash_is_global_dedup_eligible(&chunk.hash);
        JsChunkOut {
            hash: chunk.hash.hex(),
...
pub struct JsChunker {
    inner: xet_data::deduplication::Chunker,
...
impl JsChunker {
    ...
    pub fn new(target_chunk_size: usize) -> JsChunker {
        JsChunker {
            inner: xet_data::deduplication::Chunker::new(target_chunk_size),
...
pub fn compute_file_hash(chunks_array: JsValue) -> Result<String, JsValue> {
    ...
    Ok(xet_core_structures::merklehash::file_hash(&chunks).hex())
}
