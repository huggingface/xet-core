...
impl From<TokenInfo> for xet_client::cas_client::auth::TokenInfo {
...
impl xet_client::cas_client::auth::TokenRefresher for WrappedTokenRefresher {
    async fn refresh(
        &self,
    ) -> Result<xet_client::cas_client::auth::TokenInfo, xet_client::cas_client::auth::AuthError> {
        self.0
            .lock()
            .await
            .refresh_token()
            .await
            .map(xet_client::cas_client::auth::TokenInfo::from)
            .map_err(|e| xet_client::cas_client::auth::AuthError::token_refresh_failure(format!("{e:?}")))
    }
}
