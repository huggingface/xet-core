// CI smoke: concurrent multi-file uploadStream in a single XetUploadCommit
// against xet-team/xet-wasm-test on prod hub.
//
// Same shape as run-upload-multi.mjs but exercises the streaming path
// (XetStreamUpload::{write, finish}) for each of FILE_COUNT files in parallel.
//
...
if (!token) {
  console.error(
    'FAIL: HF_SMOKE_TEST_TOKEN (preferred) or HF_TOKEN required for upload-stream-multi smoke ' +
...
try {
  const result = await runBrowserSmoke({
    pagePath: 'ci-smoke/upload-stream-multi.html',
    runArg: token,
